...
public class Problem11 extends Problem {
    ...
    public String solve() {
        ...
        for (int i = 0; i < SIZE; i++) {
            final String[] nums = INPUT[i].split(" ");
            for (int j = 0; j < SIZE; j++) {
                matrix[i][j] = Integer.parseInt(nums[j]);
            }
        }

        max = Math.max(max, maxHorizontal(matrix));
        max = Math.max(max, maxVertical(matrix));

        max = Math.max(max, maxDiaganol(matrix));
        int[][] m = flip(matrix);
        max = Math.max(max, maxDiaganol(m));
        m = flip(m);
        max = Math.max(max, maxDiaganol(m));
        m = flip(m);
        max = Math.max(max, maxDiaganol(m));
    ...
    private int maxHorizontal(final int[][] pMat) {
        ...
        for (int i = 0; i < SIZE; i++) {
            for (int j = 3; j < SIZE; j++) {
                max = Math.max(max, Operation.multiplyAll(pMat[i][j], pMat[i][j - 1], pMat[i][j - 2], pMat[i][j - 3]));
            }
        }
        return max;
    }
    ...
    private int maxVertical(final int[][] pMat) {
        ...
        for (int i = 0; i < SIZE; i++) {
            for (int j = 3; j < SIZE; j++) {
                max = Math.max(max, Operation.multiplyAll(pMat[j][i], pMat[j - 1][i], pMat[j - 2][i], pMat[j - 3][i]));
            }
        }
        return max;
    }
    ...
    private int maxDiaganol(final int[][] pMat) {
        ...
        for (int i = 0; i < SIZE - 3; i++) {
            ...
            for (int j = 0, k = i; isValid(j, k); j++, k++) {
                if (n++ >= 3) {
                    max = Math.max(max, Operation.multiplyAll(pMat[j][k], pMat[j - 1][k - 1], pMat[j - 2][k - 2], pMat[j - 3][k - 3]));
                }
            }
        }
        return max;
